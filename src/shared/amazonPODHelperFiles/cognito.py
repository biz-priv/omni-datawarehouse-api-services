...
###
# Utility script to sign in to AWS cognito using SRP_A Auth Flow and optionally to set a new password
# Usage: `python3 <USERNAME> <PASSWORD> <COGNITO_POOL_ID> <COGNITO_APP_CLIENT_ID> [NEW_PASSWORD]`
import base64
import binascii
import datetime
import hashlib
...
class AWSIDP(object):
    def __init__(self, identity_pool_id, provider, id_token, pool_region=None, client=None):
        self.identity_pool_id = identity_pool_id
        self.provider = provider
        self.id_token = id_token
        self.client = client if client else boto3.client('cognito-identity', region_name=pool_region)

    def get_credentials(self, client=None):
        identity_client = self.client or client
        try:
            identity_response = identity_client.get_id(IdentityPoolId=self.identity_pool_id, Logins = {self.provider: self.id_token})
        except Exception as e:
            return {"error": True, "success": False, "message": repr(e), "data": None}
	
        identity_id = identity_response['IdentityId']
        response = identity_client.get_credentials_for_identity(IdentityId=identity_id, Logins={self.provider: self.id_token})
...
if __name__ == '__main__':
    ...
    if OPERATION == 'signin':
        ...
        USERNAME=sys.argv[4]
        RESET_CODE=sys.argv[5]
        NEW_PASSWORD=sys.argv[6]
        aws = AWSSRP(username=USERNAME, pool_id=POOL_ID, client_id=CLIENT_ID)
        aws.reset_password(reset_code=RESET_CODE, new_password=NEW_PASSWORD)
        ...
        NEW_PASSWORD=sys.argv[6]
        aws = AWSSRP(username=USERNAME, password=PASSWORD, pool_id=POOL_ID, client_id=CLIENT_ID)
        aws.set_new_password_challenge(NEW_PASSWORD)
