...
def shipmentInfo(event):
    try:
        ...
        cur.execute(f"""select distinct shipment_info.file_nbr ,shipment_info.file_date ,shipment_info.handling_stn,shipment_info.master_bill_nbr ,shipment_info.house_bill_nbr ,shipment_info.origin_port_iata ,shipment_info.destination_port_iata ,shipment_info.shipper_name ,shipment_info.consignee_name ,shipment_info.pod_date ,shipment_info.eta_date ,shipment_info.etd_date , shipment_info.schd_delv_date ,shipment_info.shipment_mode ,case when shipment_info.source_system = 'WT' then m.order_status else shipment_info.current_status end as order_status,case when shipment_info.source_system = 'WT' then m.order_Status_Desc else shipment_info.current_status end as order_status_desc,customers.name from shipment_info left outer join customers on shipment_info.source_system = customers.source_system and trim(shipment_info.bill_to_nbr) = trim(customers.nbr) left outer join(select shipment_milestone.source_system ,shipment_milestone.file_nbr ,shipment_milestone.order_status ,shipment_milestone.order_status_desc from shipment_milestone a join (select file_nbr ,max(event_seq_nbr)event_seq_nbr  from shipment_milestone where is_public = 'Y' group by file_nbr)b on shipment_milestone.file_nbr = b.file_nbr and shipment_milestone.event_seq_nbr = b.event_seq_nbr and shipment_milestone.is_public  = 'Y')m on shipment_info.source_system = m.source_system and shipment_info.file_nbr = m.file_nbr where house_bill_nbr = '{x}'""")
        con.commit()
