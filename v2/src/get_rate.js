...
module.exports.handler = async (event, context, callback) => {
  ...
  try {
    ...
    if (
      get(event, `enhancedAuthContext.customerId`, "") !=
      "customer-portal-admin"
    ) {
      customerNumber = await getCustomerNumber(apiKey, callback);
    ...
    if (
      ...
      !Number.isInteger(
        Number(get(body, "shipmentRateRequest.shipperZip", ""))
      ) ||
      !Number.isInteger(
        Number(get(body, "shipmentRateRequest.consigneeZip", ""))
      )
    ) {
      ...
      if (responseData.length == 0) {
        valError =
          "customer doen't exist. Please contact support for further details.";
      } else {
        const pickupTime = new Date(
          get(body, "shipmentRateRequest.pickupTime", "")
        );
      ...
      reqFields.consigneeZip = get(
        body,
        "shipmentRateRequest.consigneeZip",
        ""
      );
      reqFields.pickupTime = get(
        body,
        `shipmentRateRequest.pickupTime`,
        ""
      ).replace("Z", "+00:00");
      ...
      for (
        let i = 0;
        i < get(body, "shipmentRateRequest.shipmentLines.length", 0);
        i++
      ) {
        reqFields.shipmentLines.push({});
        for (let key in get(body, `shipmentRateRequest.shipmentLines[${i}]`)) {
          if (!key.includes("//")) {
            reqFields.shipmentLines[i][key] = get(
              body,
              `shipmentRateRequest.shipmentLines[${i}][${key}]`
            );
          }
        }
      }
    ...
    if (valError) {
      ...
      if (error.toString().includes("shipmentLines")) {
        return callback(
          response(
            ...
            "shipmentLines." +
              key +
              get(error, `details[0].message`, "").split('"')[2]
          )
        );
      } else {
      ...
      newJSON.RatingInput.PickupTime = get(
        reqFields,
        `pickupTime`,
        ""
      ).toString();
      newJSON.RatingInput.PickupDate = get(
        reqFields,
        `pickupTime`,
        ""
      ).toString();
      newJSON.RatingInput.PickupLocationCloseTime = get(
        reqFields,
        `pickupTime`,
        ""
      ).toString();
    ...
    if (
      ...
      !isNaN(Number(body.shipmentRateRequest.commodityClass))
    ) {
    ...
    if ("accessorialList" in body.shipmentRateRequest) {
      ...
      for (
        ...
        newJSON.AccessorialInput.AccessorialInput.push({
          AccessorialCode: get(
            body,
            `shipmentRateRequest.accessorialList[${x}]`
          ),
        });
      }
    }
    ...
    log(
      correlationId,
      JSON.stringify(get(newJSON, `AccessorialInput`, "")),
      200
    );
    ...
    if ("Error" in get(dataObj, `shipmentRateResponse`, "")) {
      return callback(
        response("[400]", get(dataObj, `shipmentRateResponse.Error`, ""))
      );
    } else {
    ...
    try {
      ...
      return callback(response("[400]", err ?? get(err, `message`, error)));
    }
    return callback(response("[400]", error ?? get(error, `message`, error)));
  }
};
...
function addCommodityWeightPerPiece(inputData) {
  ...
  for (let shipmentLine of get(inputData, `shipmentLines`, [])) {
    ...
    if (shipmentLine.dimUOM.toLowerCase() == "cm") {
      shipmentLine.length = Math.round(
        get(shipmentLine, `length`, 0) * 0.393701
      );
      shipmentLine.width = Math.round(get(shipmentLine, `width`, 0) * 0.393701);
      shipmentLine.height = Math.round(
        get(shipmentLine, `height`, 0) * 0.393701
      );
    }
    if (shipmentLine.weightUOM.toLowerCase() == "kg") {
      shipmentLine.weightPerPiece = Math.round(
        (get(shipmentLine, `weight`, 0) * 2.2046) /
          get(shipmentLine, `pieces`, 1)
      );
    } else {
      shipmentLine.weightPerPiece = Math.round(
...
function makeXmlToJson(data) {
  try {
    ...
    if (
      get(
        obj,
        `soap:Envelope.soap:Body.GetRatingByCustomerResponse.GetRatingByCustomerResult`,
        ""
      ).hasOwnProperty("RatingOutput")
    ) {
      const modifiedObj = get(
        obj,
        `soap:Envelope.soap:Body.GetRatingByCustomerResponse.GetRatingByCustomerResult.RatingOutput`,
        ""
      );
      ...
      if (isArray(modifiedObj)) {
        ...
        return modifiedObj.map((e) => {
          log(
            correlationId,
            JSON.stringify(get(e, `AccessorialOutput`, "")),
            200
          );
          ...
          if (
            ...
            AccessorialOutput = getAccessorialOutput(
              get(e, `AccessorialOutput`, null)
            );
          }
          let EstimatedDelivery;
          if (
            get(e, `DeliveryTime`, null) !== null &&
            get(e, `DeliveryTime`, null) != null
          ) {
          ...
          return {
            ...
            totalRate: parseFloat(
              get(e, `StandardTotalRate`, "").replace(/,/g, "")
            ),
        ...
        if (
          ...
          for (
            ...
            get(
              ...
              : get(
                  modifiedObj,
                  `AccessorialOutput.AccessorialOutput[${i}]
                  .AccessorialDesc`,
                  ""
                )
        ...
        let EstimatedDelivery;
        if (
          get(modifiedObj, `DeliveryTime`, null) !== null &&
          get(modifiedObj, `DeliveryTime`, "") != null
        ) {
          // EstimatedDelivery = new Date(modifiedObj.DeliveryDate);
          console.info("EstimatedDelivery=========>");
          //----------------------------------------------------------------
          const dateStr = JSON.stringify(
            get(modifiedObj, `DeliveryDate`, "") +
              " " +
              get(modifiedObj, `DeliveryTime`, "")
          );
          const dateObj = moment(dateStr, "M/D/YYYY h:mm:ssA");
          const deliveryStr = dateObj.format("YYYY-MM-DDTHH:mm:ss");
          EstimatedDelivery = deliveryStr;
          ...
          return [
            {
              ...
              estimatedDelivery:
                get(modifiedObj, `DeliveryDate`, "") == "1/1/1900"
                  ? ""
                  : EstimatedDelivery,
              totalRate: parseFloat(
...
function getAccessorialOutput(AccessorialOutput) {
  ...
  if (Array.isArray(get(AccessorialOutput, `AccessorialOutput`, ""))) {
    if (get(AccessorialOutput, `AccessorialOutput`, "")) {
      for (
        let i = 0;
        i < get(AccessorialOutput, `AccessorialOutput.length`, "");
        i++
      ) {
        list[i] = {};
        list[i].code = get(
          AccessorialOutput,
          `AccessorialOutput[${i}].AccessorialCode`,
          ""
        );
        list[i].description = get(
          AccessorialOutput,
          `AccessorialOutput[${i}].AccessorialDesc`,
          ""
        );
        list[i].charge = parseFloat(
          get(
            AccessorialOutput,
            `AccessorialOutput[${i}].AccessorialCharge`,
            ""
          ).replace(/,/g, "")
        );
      }
    ...
    list[i].code = get(
      AccessorialOutput,
      `AccessorialOutput.AccessorialCode`,
      ""
    );
    list[i].description = get(
      AccessorialOutput,
      `AccessorialOutput.AccessorialDesc`,
      ""
    );
    list[i].charge = parseFloat(
      get(AccessorialOutput, `AccessorialOutput.AccessorialCharge`, "").replace(
        /,/g,
        ""
      )
    );
...
async function getRating(postData) {
  try {
    ...
    if (res.status == 200) {
      ...
      throw get(res, `response.statusText`, "");
    }
  } catch (e) {
    let obj = convert(get(e, `response.data`, ""), { format: "object" });
    let errorMessage = get(
      obj,
      `soap:Envelope.soap:Body.soap:Fault.soap:Reason.soap:Text.#`,
      ""
    );
...
async function getCustomerNumber(xApiKey, callback) {
  try {
    const documentClient = new AWS.DynamoDB.DocumentClient({
      region: process.env.REGION,
      functionName: process.env.FUNCTION_NAME,
    ...
    if (
      get(validatorResp, `Items`, null) !== null &&
      get(validatorResp, `Items.length`, 0) > 0
    ) {
