...
let valError;
  ...
  if (
    ...
    !("housebill" in event.enhancedAuthContext)
  ) {
    valError = "housebill not found.";
    ...
    !("docType" in body.shipmentRateRequest)
  ) {
    valError =
...
async function getdocument() {
    let res;
    try {
      res = await axios.get( process.env.GETDOCUMENT_API+"/housebill=6978596/doctype=housebill%7Cdoctype=label%7Cdoctype=hcpod", {
        headers: {
          Accept: "text/xml",
