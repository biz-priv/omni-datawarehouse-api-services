  ...
  try {
    ...
    await Promise.all(
      newData.map(async (item) => {
         ...
         */
        const xmlPayload = makeJsonToXml(payload, item);
...
async function getDataFromDB(data = null) {
  try {
    ...
    c.ref_nbr
        ...
        and b.order_status in ('PUP','COB','DEL')`;
...
function makeJsonToXml(payload, inputData) {
  ...
  transBody["otm:ShipmentGid"]["otm:Gid"]["otm:Xid"] = inputData.house_bill_nbr;
